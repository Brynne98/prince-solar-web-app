-- ============================================================================
-- soc_overnight was firing on whatever appliance happened to be on.
--
-- The old rule read batt_w from the single newest agg_minute row and treated
-- that instant as the draw for the rest of the night. Any heavy load became a
-- prediction: a kettle, an oven, and above all the 04:00 geyser, which pulls
-- ~8 kW against a ~120 W overnight baseline and runs on a timer.
--
-- Replayed over the 68 nights of history to 5 Aug 2026, the old rule fired on
-- 63 of them and 51 were false -- most of those the same 04:00 geyser, on a
-- battery that finished the night above 70%.
--
-- Two changes: the draw is now a 60-minute MEDIAN, and the shortfall must hold
-- for 30 continuous minutes before it counts. Same replay: 23 nights, 6 false,
-- and all 12 nights that really reached the 20% reserve still caught.
--
-- The projection therefore has to be evaluated at every minute of the window
-- rather than once, which is why `overnight` is now an aggregate. Same shape as
-- api_balance's sustained-spread check.
-- ============================================================================
  ...
  night_calc as (
    select
      ...
      case when w.draw_w < -50
           then greatest(0.0, (coalesce(w.soc, 0) - 20) / 100.0 * 26.5)
                / (abs(w.draw_w) / 1000.0)
