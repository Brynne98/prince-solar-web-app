-- ============================================================================
-- GET /api/overview — the live snapshot behind the main tab.
--
-- The one real behavioural change in the whole port: Express fetches all five
-- SunSynk realtime endpoints per inverter on every page load, while this reads the
-- newest logged row. Up to 60 s staler, and the reason the browser needs no SunSynk
-- credentials at all — which was the hard requirement.
--
-- SECURITY DEFINER because inverter identity (alias, model, firmware, plant name)
-- lives in private.meta. It returns only that descriptive metadata — never a token.
-- ============================================================================
