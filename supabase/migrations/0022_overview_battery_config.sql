-- ============================================================================
-- Publish the pack configuration on the snapshot the dashboard already polls.
--
-- 0020 made app_config authoritative for the backend, but the frontend still
-- kept its own editable copy in localStorage (public/app.jsx DEFAULT_SETTINGS),
-- so the two could still disagree -- the dashboard would follow the slider
-- while the phone alerts followed the table.
--
-- Rather than a new RPC, hang it off api_overview: the Live tab polls that
-- every 60s already, so this costs no extra round trip. The frontend then drops
-- its own copies and renders these read-only, leaving exactly one editable
-- copy of each number -- the app_config row.
--
-- JSON keys mirror the frontend's names (battCapacity / reserve) rather than
-- the app_config keys, so the JSX reads the same as it did before.
-- ============================================================================
