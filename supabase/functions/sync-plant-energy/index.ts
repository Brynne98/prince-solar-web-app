// `sync-plant-energy` — cache SunSynk's plant-level kWh totals into Postgres, per plant.
//
...
// is ample and cheap: ~9 API calls per plant.
...
async function syncPlant(job: PlantJob, dailyMonths: number, started: number) {
  ...
  const { data: gm, error: gmErr } = await db.rpc("q_grid_feed_scale", { p_plant: plantId });
  ...
  for (let k = 0; k < dailyMonths; k++) {
    ...
    const mo = (mAbs % 12) + 1;
    const rows = await fetchMonthRows(account, plantId, yr, mo, gridMul);
  ...
  return { plantId, gridMul, rows: upserts.length, years: synced.years, months: synced.months };
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const job of jobs) {
      ...
      try {
        const r = await syncPlant(job, dailyMonths, started);
        rows += r.rows;
        plants.push(r);
      } catch (e) {
