...
4. **Yesterday read as a four-hour stub between midnight and 04:15** — FIXED 2026-09-19 by migration `0058_finished_day_rows.sql`. `plant_energy` day rows come from SunSynk's plant feed via `sync-plant-energy`, on a single daily cron at `15 2 * * *` = **04:15 SAST** (`supabase/migrations/0009_schedule.sql:59`). That run writes the *current* day as a partial: at 04:15 it is four hours of overnight load and no sun. Nothing overwrites it until the next run, 24 h later.
   - `api_trends_daily` computes **today** live from `agg_minute` (migration 0013) but reads `plant_energy` for every `period < today`. So from local midnight until 04:15, the Solar tab's 30-day bar chart shows *yesterday* as that stub — a near-zero bar, which reads as a missing bar.
   - Measured on plant 538820: `plant_energy` 2026-09-18 = `pv 0, load 3, imp 0`, `synced_at 2026-09-18 02:15 UTC`; `agg_minute` for the same day = 1440 minutes, `pv 20.0, load 32.9, imp 24.1`. The minute data was complete and correct the whole time.
   - **`api_energy` is worse**: its week and month buckets sum `plant_energy` day rows with no `agg_minute` branch at all, so **This week** and **This month** carry the stub for yesterday *and* miss today's generation entirely, all day, until the 04:15 sync.
   - **The fix (0058):** a cached day counts as finished only when its `synced_at` is past the end of that day in the plant's zone (and its `pv` series is not null). Days without a finished row come from `agg_minute`. One helper, `public._plant_day_energy(plant, from, to)`, holds the rule, and all three readers call it — `api_trends_daily`, `api_energy` (week/month; year and lifetime stay on month buckets), and `api_trends_compare` (both sides of every pair). It is the same flip 0013 made for today, delayed until the vendor row exists instead of flipping to a stub, so a day's source changes once and in one direction.
   - **Rejected:** "prefer `agg_minute` for any day with ≥1400 of 1440 minutes" (Grok and Fable both, 2026-09-19). A gappy day *plus* a stub row falls back to the stub, under-reporting worse than the gaps; and a day's source would flip weeks later when `recover` fills a hole, moving old bars and drifting the week total.
   - Accepted, unchanged from 0013: at 04:15 a day steps as it moves from our integral to the vendor total — the cache scales grid by `q_grid_feed_scale` where `agg_minute` sums each inverter's measured watts, so ~1% on pv/load and more on import/export. Today's bar already does this at midnight.
   - Also fixed by the same change: `api_energy`'s week and month now include today at all, and the compare arrows no longer droop for four hours.
   - It self-corrected every morning at 04:15 and returned every midnight, which is why it went unnoticed for so long: the numbers were right whenever anyone normally looked.
